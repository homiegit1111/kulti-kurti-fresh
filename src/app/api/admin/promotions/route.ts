/**
 * Admin offers API — list + create.
 *
 *   GET  /api/admin/promotions
 *        → { promotions: AdminPromotion[] }   newest first, each with a derived `state`
 *   POST /api/admin/promotions   application/json
 *        → { promotion: AdminPromotion }      201
 *
 * See docs/ADMIN_API_CONTRACT.md §6. Admin-gated + service-role: RLS only lets
 * the browser read AUTOMATIC, currently-live promotions, because a coupon code
 * is a secret and public-read would make every unreleased code enumerable.
 *
 * The validation below duplicates the copy in ./[id]/route.ts on purpose: a
 * route file may not export anything but its HTTP methods, so the two handlers
 * cannot share a parser without a third module. Keep them in step.
 */
...
const BLANK_DRAFT: PromotionDraft = {
  code: null,
  title: "",
  // Placeholder only — create rejects a body with no `kind`, see parsePromotion.
  kind: "percent",
  description: "",
  value_percent: null,
  value_inr: null,
  ...
  starts_at: null,
  ends_at: null,
  // Matches the column default: a new offer never goes live on its own.
  is_active: false,
  max_redemptions: null,
  max_redemptions_per_buyer: null,
  badge_label: "",
  priority: 0,
};
...
/**
 * Validate a create (existing = null) or a patch (existing = the current row) and
 * return the full row to write. Every rule the database enforces is checked here
 * first, so a bad combination surfaces as a sentence the owner can act on rather
 * than "violates constraint commerce_promotions_value_shape".
 */
function parsePromotion(
  body: Record<string, unknown>,
  existing: PromotionDraft | null,
): ParseResult<PromotionDraft> {
  const base = existing ?? BLANK_DRAFT;
  ...
  if (has(body, "kind")) {
    ...
    if (raw !== "percent" && raw !== "flat_inr" && raw !== "free_shipping") {
      return { ok: false, error: kindMessage };
    }
    kind = raw;
  } else if (!existing) {
    return { ok: false, error: kindMessage };
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  const guard = await guardAdmin(req, {
    ...
    rateLimit: { name: "admin-offers-read", limit: 120 },
  });
  ...
  if (error) {
    console.error("[admin-promotions] list failed:", error.message);
    return serverError("Could not load your offers. Please refresh in a moment.");
  }

  const now = Date.now();
  const rows: PromotionRow[] = data ?? [];
  return NextResponse.json({
    promotions: rows.map((row) => serializePromotion(row, now)),
  });
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const parsed = parsePromotion(parsedBody.body, null);
  if (!parsed.ok) return badRequest(parsed.error);

  const { data, error } = await guard.ctx.db
    .from("commerce_promotions")
    .insert({ ...parsed.value, created_by: guard.ctx.userId })
    .select(PROMOTION_COLUMNS)
    .single();
  ...
  if (error || !data) {
    ...
    console.error("[admin-promotions] create failed:", error?.message);
    return serverError("Could not save the offer. Please try again.");
  }

  const created: PromotionRow = data;
  ...
  return NextResponse.json(
    { promotion: serializePromotion(created, Date.now()) },
    { status: 201 },
  );
}
